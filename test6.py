    ...
    for i in range(1, max_times):
        for j in range(1, max_times):
            for k in range(1, max_times):
                for l in range(1, max_times):
                    result = i*data[1]+j*data[2]+k*data[3]+l*data[4]
                    if data[0] == result:
                        result_list = [i, j, k, l]
                        return result_list
    return 0
    ...
    for i in range(xyz_len):
        for j in range(i+1, xyz_len):
            for k in range(j+1, xyz_len):
                for l in range(k+1, xyz_len):
                    data = [round(float(m[0]), 6), round(float(data_xyz[i][0]), 6), round(float(data_xyz[j][0]), 6), round(float(data_xyz[k][0]), 6), round(float(data_xyz[l][0]), 6)]
                    result = find_abcd(data)
                    if result != 0:
                        final_result.append([m[0], data_xyz[i][1], data_xyz[j][1], data_xyz[k][1], data_xyz[l][1]]+result+["", "", data_xyz[i][0], data_xyz[j][0], data_xyz[k][0]], data_xyz[l][0])
...
def main():
    ...
    test_data = []
    header = ["m", "x", "y", "z", "u", "a", "b", "c", "d", "", "", "x", "y", "z", "u"]
